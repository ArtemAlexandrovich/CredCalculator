...
function countCreditWrite() {
    ...
    for (let i = 0; i < (period); i++) {
        ...
        payment_arr.push([bestDate, (monthPay.toFixed(2)).replace(/[.]/, ','), (In.toFixed(2)).replace(/[.]/, ','), (S.toFixed(2)).replace(/[.]/, ','), (Sn2.toFixed(2)).replace(/[.]/, ',')]);

    }

    payment_arr.push(["Итого", ((period * monthPay).toFixed(2)).replace(/[.]/, ','), (allPercents.toFixed(2)).replace(/[.]/, ','), (sum.toFixed(2)).replace(/[.]/, ','), ""]);
...
function addTableToDoc() {
    ...
    let str_inner = '<table border="1" id = "table_id"> ' +
        '<tr> ' +
            '<th>Дата</th> ' +
